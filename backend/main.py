...
from backend.utilities.user_auth import is_user_admin_in_db, is_logged_in
...
@app.get("/customers")
async def get_customers(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
@app.get("/titles")
async def get_titles(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
@app.get("/countries")
async def get_countries(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
@app.get("/currencies")
async def get_currencies(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
async def get_payment_methods(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
async def get_transaction_statuses(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
async def get_mfa_statuses(jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
async def is_admin(jwt_token: str):
    return is_user_admin_in_db(get_username_from_jwt(jwt_token))

# Post APIs
@app.post("/createtransaction")
async def post_transactions(transaction: Transaction, jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
...
async def delete_transaction(transaction_id: str, jwt_token: str):
    ...
    if not is_user_admin_in_db(username):
...
async def update_transaction(transaction_id: str, transaction: Transaction, jwt_token: str):
    if not is_logged_in(jwt_token):
        return "User is not logged in"
